...
{
    nob_sb_append_cstr(content, "#define PM_EVENTS_CFG_FILE PmEvents.cfg\n");
    nob_sb_append_cstr(content, "#define PM_EVENTS_FORMAT_CFG_FILE PmEventFormat.cfg\n");
    nob_sb_append_cstr(content, "#define PM_EVENTS_PARAMS_CFG_FILE PmEventParams.cfg\n");
}
...
int main(int argc, char **argv)
{
    ...
    if (config_exists == 0)
    {
        ...
        if (!nob_write_entire_file(CONFIG_PATH, content.items, content.count))
            return 1;
    }
    else
    {
